 ...
 * from https://docs.chain.link/ccip/tutorials/evm/send-arbitrary-data
 */
...
contract NFTPoolLockAndRelease is CCIPReceiver, OwnerIsCreator {
    ...
    function lockAndSendNft(uint256 tokenId, address newOwner,
        uint64 chainSelector, address receiver) public returns(bytes32) {
        nft.transferFrom(msg.sender, address(this), tokenId);
        bytes memory payload = abi.encode(tokenId, newOwner);
        bytes32 msgId = sendMessagePayLINK(chainSelector, receiver, payload);
        ...
        ) // Make sure source chain and sender are allowlisted
    {
        ...
        // check if nft locked. not necessarily check because transferFrom will check whether
        // nft belongs to contract
